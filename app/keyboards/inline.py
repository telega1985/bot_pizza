...
def get_user_catalog_btns(*, level: int, categories: list, sizes: tuple[int] = (2,)):
    ...
    for category in categories:
        keyboard.add(InlineKeyboardButton(
            text=category.name, callback_data=MenuCallback(
...
def get_products_btns(
    ...
    keyboard.add(InlineKeyboardButton(
        text="Назад", callback_data=MenuCallback(level=level - 1, menu_name="catalog").pack()
    ))
    keyboard.add(InlineKeyboardButton(
        text="Корзина 🧺", callback_data=MenuCallback(level=3, menu_name="cart").pack()
    ))
    keyboard.add(InlineKeyboardButton(
        text="Купить 💳", callback_data=MenuCallback(
    ...
    for text, menu_name in pagination_btns.items():
        if menu_name == "next":
            row.append(InlineKeyboardButton(
                text=text,
                callback_data=MenuCallback(level=level, menu_name=menu_name, category=category, page=page + 1).pack()
            ))
